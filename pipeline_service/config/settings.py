...
class Settings(BaseSettings):
    ...
    # Trellis settings - Detail Bias
    trellis_model_id: str = Field(default="jetx/trellis-image-large", env="TRELLIS_MODEL_ID")
    trellis_sparse_structure_steps: int = Field(default=18, env="TRELLIS_SPARSE_STRUCTURE_STEPS")
    trellis_sparse_structure_cfg_strength: float = Field(default=6.0, env="TRELLIS_SPARSE_STRUCTURE_CFG_STRENGTH")
    trellis_slat_steps: int = Field(default=32, env="TRELLIS_SLAT_STEPS")
    trellis_slat_cfg_strength: float = Field(default=5.2, env="TRELLIS_SLAT_CFG_STRENGTH")
    ...
    output_image_size: tuple[int, int] = Field(default=(640, 640), env="OUTPUT_IMAGE_SIZE") # (height, width)
    padding_percentage: float = Field(default=0.18, env="PADDING_PERCENTAGE")
